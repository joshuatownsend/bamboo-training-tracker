...
-- Update the trigger function to use both service role and anon key with better error handling
...
BEGIN
  ...
  -- Include version check in the URL to confirm which version is running
  SELECT 
    status_code, content, request_id 
  INTO 
...
-- Create a function to check the version of the deployed edge function
CREATE OR REPLACE FUNCTION public.check_edge_function_version(function_name text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  BEGIN
    auth_header := 'Bearer ' || current_setting('supabase.service_role_key', true);
  EXCEPTION WHEN OTHERS THEN
    BEGIN
      auth_header := 'Bearer ' || current_setting('supabase.anon_key', true);
    EXCEPTION WHEN OTHERS THEN
      RETURN jsonb_build_object('success', false, 'error', 'Could not retrieve authentication keys');
    END;
  END;

  -- Call the version endpoint of the edge function
  SELECT 
    content::jsonb INTO result
  FROM 
    net.http_get(
      url := 'https://fvpbkkmnzlxbcxokxkce.supabase.co/functions/v1/' || function_name || '/version',
      headers := jsonb_build_object(
        'Authorization', auth_header,
        'Content-Type', 'application/json'
      )
...
-- Create a view for training completions stats to help with diagnostics
CREATE OR REPLACE VIEW public.training_completions_stats AS
SELECT
  COUNT(*) as total_completions,
  COUNT(DISTINCT employee_id) as employees_with_completions,
  COUNT(DISTINCT training_id) as unique_trainings,
  MIN(completion_date) as earliest_completion,
  MAX(completion_date) as latest_completion,
  MAX(created_at) as latest_record_created,
  MAX(updated_at) as latest_record_updated
FROM
  public.employee_training_completions;

-- Update sync_status table to ensure it has a details column for extra information
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.sync_status ADD COLUMN details JSONB;
  END IF;
END $$;

-- Add indices to improve query performance on the completions table
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'employee_training_completions' 
    AND indexname = 'idx_employee_training_completions_employee_id'
  ) THEN
    CREATE INDEX idx_employee_training_completions_employee_id 
    ON public.employee_training_completions(employee_id);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'employee_training_completions' 
    AND indexname = 'idx_employee_training_completions_training_id'
  ) THEN
    CREATE INDEX idx_employee_training_completions_training_id 
    ON public.employee_training_completions(training_id);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'employee_training_completions' 
    AND indexname = 'idx_employee_training_completions_completion_date'
  ) THEN
    CREATE INDEX idx_employee_training_completions_completion_date 
    ON public.employee_training_completions(completion_date);
  END IF;
END $$;

COMMENT ON FUNCTION public.trigger_training_completions_sync() IS 'Improved function to trigger the training completions sync with better error handling and logging';
COMMENT ON FUNCTION public.check_edge_function_version(text) IS 'Function to check the version of a deployed edge function';
COMMENT ON VIEW public.training_completions_stats IS 'Statistics on training completions for diagnostics';
